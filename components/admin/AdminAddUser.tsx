...
export default function AdminAddUser({ onBack, onUserAdded }: AdminAddUserProps) {
  ...
  const validateForm = () => {
    ...
    if (formData.role === 'pharmacist') {
      if (!formData.pharmacistLicenseNumber.trim()) {
        newErrors.pharmacistLicenseNumber = 'License number is required';
      }
    }

    if (formData.role === 'lab_technician') {
      if (!formData.labTechnicianSpecialization.trim()) {
        newErrors.labTechnicianSpecialization = 'Specialization is required';
      }
      if (!formData.labTechnicianLicenseNumber.trim()) {
        newErrors.labTechnicianLicenseNumber = 'License number is required';
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('[AdminAddUser] Creating user via:', url);
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          role: formData.role,
          nic: formData.role === 'patient' ? formData.nic : undefined,
          rfid: formData.role === 'patient' ? formData.rfid : undefined,
          dateOfBirth: formData.role === 'patient' ? formData.dateOfBirth : undefined,
          gender: formData.role === 'patient' ? formData.gender : undefined,
          phoneNumber: formData.role === 'patient' ? formData.phoneNumber : undefined,
          address: formData.role === 'patient' ? formData.address : undefined,
          bloodType: formData.role === 'patient' ? formData.bloodType : undefined,
          allergies: formData.role === 'patient' ? formData.allergies : undefined,
          pharmacistLicenseNumber: formData.role === 'pharmacist' ? formData.pharmacistLicenseNumber : undefined,
          labTechnicianSpecialization:
            formData.role === 'lab_technician' ? formData.labTechnicianSpecialization : undefined,
          labTechnicianLicenseNumber:
            formData.role === 'lab_technician' ? formData.labTechnicianLicenseNumber : undefined,
          ...
          qualifications: formData.role === 'doctor' ? formData.qualifications : undefined,
          yearsOfExperience: formData.role === 'doctor' ? formData.yearsOfExperience : undefined,
        }),
      });
      ...
      if (response.ok && data?.success) {
        console.log('[AdminAddUser] Create user response:', data);
    ...
    >
      ...
      >
            ...
            <View style={styles.roleMenu}>
              {roles.map((role) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setFormData((prev) => {
                      const nextRole = role.value as any;
                      const next: any = { ...prev, role: nextRole };

                      // Clear doctor-only fields when switching away from doctor
                      if (prev.role === 'doctor' && nextRole !== 'doctor') {
                        next.specialization = '';
                        next.licenseNumber = '';
                        next.qualifications = '';
                      ...
                      // Clear patient-only fields when switching away from patient
                      if (prev.role === 'patient' && nextRole !== 'patient') {
                        next.nic = '';
                        next.rfid = '';
                        next.dateOfBirth = '';
                        next.gender = '';
                      ...
                      // Clear pharmacist-only fields when switching away from pharmacist
                      if (prev.role === 'pharmacist' && nextRole !== 'pharmacist') {
                        next.pharmacistLicenseNumber = '';
                      }

                      // Clear lab technician-only fields when switching away from lab technician
                      if (prev.role === 'lab_technician' && nextRole !== 'lab_technician') {
                        next.labTechnicianSpecialization = '';
                        next.labTechnicianLicenseNumber = '';
                      }
        ...
        {formData.role === 'patient' && (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <MaterialCommunityIcons name="account-heart" size={24} color="#3B82F6" />
            ...
            <View style={styles.inputGroup}>
              <RNText style={styles.label}>Contact Info</RNText>
              <TextInput
                style={styles.inputContainer}
        ...
        {formData.role === 'doctor' && (
          <View style={styles.card}>
            ...
            <View style={styles.inputGroup}>
              <RNText style={styles.label}>License Number</RNText>
              <TextInput
                style={styles.inputContainer}
        ...
        {/* Pharmacist Information (only for pharmacists) */}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) => {
                  ...
                  if (errors.pharmacistLicenseNumber) {
                    setErrors({ ...errors, pharmacistLicenseNumber: '' });
                  }
                }}
              />
              {errors.pharmacistLicenseNumber && (
                <RNText style={styles.errorText}>{errors.pharmacistLicenseNumber}</RNText>
              )}
            </View>
          </View>
        )}

        {/* Lab Technician Information (only for lab technicians) */}
        {formData.role === 'lab_technician' && (
          <View style={styles.card}>
            ...
            <View style={styles.inputGroup}>
              <RNText style={styles.label}>Specialization *</RNText>
              <TextInput
                style={[styles.inputContainer, errors.labTechnicianSpecialization && styles.inputError]}
                placeholder="Enter specialization"
                placeholderTextColor="#9CA3AF"
                value={formData.labTechnicianSpecialization}
                onChangeText={(text) => {
                  setFormData({ ...formData, labTechnicianSpecialization: text });
                  if (errors.labTechnicianSpecialization) {
                    setErrors({ ...errors, labTechnicianSpecialization: '' });
                  }
                }}
              />
              {errors.labTechnicianSpecialization && (
                <RNText style={styles.errorText}>{errors.labTechnicianSpecialization}</RNText>
              )}
            </View>
            ...
            <View style={styles.inputGroup}>
              <RNText style={styles.label}>License Number *</RNText>
              <TextInput
                style={[styles.inputContainer, errors.labTechnicianLicenseNumber && styles.inputError]}
                placeholder="Enter license number"
                placeholderTextColor="#9CA3AF"
                value={formData.labTechnicianLicenseNumber}
                onChangeText={(text) => {
                  setFormData({ ...formData, labTechnicianLicenseNumber: text });
                  if (errors.labTechnicianLicenseNumber) {
                    setErrors({ ...errors, labTechnicianLicenseNumber: '' });
                  }
                }}
              />
              {errors.labTechnicianLicenseNumber && (
                <RNText style={styles.errorText}>{errors.labTechnicianLicenseNumber}</RNText>
              )}
            </View>
          </View>
        )}
